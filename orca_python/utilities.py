...
class Utilities:
                ...
                for part_idx, partition in dataset:
                    ...
                    for metric_name in self.general_conf["metrics"]:
                        try:
                            ...
                            raise AttributeError("No metric named '%s'" % metric_name)
    ...
    def _load_dataset(self, dataset_path):
        ...
        try:
            ...
            for filename in os.listdir(dataset_path):
                if filename.startswith("train_"):
                    train_inputs, train_outputs = self._read_file(
                        os.path.join(dataset_path, filename)
                    )
                    partition_list[filename[filename.find(".") + 1 :]][
                        "train_inputs"
                    ] = train_inputs
                    partition_list[filename[filename.find(".") + 1 :]][
                        "train_outputs"
                    ...
                    test_inputs, test_outputs = self._read_file(
                        os.path.join(dataset_path, filename)
                    )
                    partition_list[filename[filename.find(".") + 1 :]][
                        "test_inputs"
            ...
            raise RuntimeError(
                "Found partition without train files: partition %s"
                % filename[filename.find(".") + 1 :]
            )
