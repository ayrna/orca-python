...
class NNPOM(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        Y = 1 * (
            np.tile(y, (1, num_labels))
            == np.tile(np.arange(1, num_labels + 1)[np.newaxis, :], (m, 1))
        )

        # Hidden layer weigths (with bias)
        initial_Theta1 = self.__randInitializeWeights(
            input_layer_size + 1, self.getHiddenN()
        )
        ...
        results_optimization = scipy.optimize.fmin_l_bfgs_b(
            func=self.__nnPOMCostFunction,
            x0=initial_nn_params.ravel(),
            args=(input_layer_size, self.hiddenN, num_labels, X, Y, self.lambdaValue),
            fprime=None,
            factr=1e3,
        ...
        Theta1, Theta2, thresholds_param = self.__unpackParameters(
            self.nn_params, input_layer_size, self.getHiddenN(), num_labels
        )
    ...
    def predict(self, test):
        ...
        z3 = np.tile(self.thresholds, (m, 1)) - np.tile(
            projected, (1, self.num_labels - 1)
        )
    ...
    def __unpackParameters(
        self, nn_params, input_layer_size, hidden_layer_size, num_labels
    ):
        """
        ...
        Theta1 = np.reshape(
            nn_params[0:nTheta1], (hidden_layer_size, (input_layer_size + 1)), order="F"
        )

        nTheta2 = hidden_layer_size
        Theta2 = np.reshape(
            nn_params[nTheta1 : (nTheta1 + nTheta2)], (1, hidden_layer_size), order="F"
        )

        thresholds_param = np.reshape(
            nn_params[(nTheta1 + nTheta2) :], ((num_labels - 1), 1), order="F"
        )
    ...
    def __randInitializeWeights(self, L_in, L_out):
        ...
        W = (
            np.random.rand(L_out, L_in) * 2 * self.getEpsilonInit()
            - self.getEpsilonInit()
        )
    ...
    def __convertThresholds(self, thresholds_param, num_labels):
        ...
        thresholds = np.reshape(
            np.multiply(
                ...
                np.tril(np.ones((num_labels - 1, num_labels - 1))),
            ).sum(axis=1),
            (num_labels - 1, 1),
        ).T
    ...
    def __nnPOMCostFunction(
        ...
        z3 = np.tile(thresholds, (m, 1)) - np.tile(
            np.matmul(a2, Theta2.T), (1, num_labels - 1)
        )
        a3T = 1.0 / (1.0 + np.exp(-z3))
        a3 = np.append(a3T, np.ones((m, 1)), axis=1)
        h = np.concatenate(
        ...
        # Cross entropy
        J = np.sum(-np.log(out[np.where(Y == 1)]), axis=0) / m + lambdaValue * p / (
            2 * m
        )

        # Cross entropy
        errorDer = np.zeros(Y.shape)
        errorDer[np.where(Y != 0)] = np.divide(
            -Y[np.where(Y != 0)], out[np.where(Y != 0)]
        )

        # Calculate sigmas
        fGradients = np.multiply(a3T, (1 - a3T))
        gGradients = np.multiply(
            errorDer,
            np.concatenate(
                (
                    fGradients[:, 0].reshape(-1, 1),
                    (fGradients[:, 1:] - fGradients[:, :-1]),
                    -fGradients[:, -1].reshape(-1, 1),
                ),
                axis=1,
            ),
        )
        ...
        p1 = (lambdaValue / m) * np.concatenate(
        ...
        # Treshold gradients
        ThreshGradMatrix = np.multiply(
            np.concatenate(
                (
                    np.triu(np.ones((num_labels - 1, num_labels - 1))),
                    np.ones((num_labels - 1, 1)),
                ),
                axis=1,
            ),
            np.tile(gGradients.sum(axis=0), (num_labels - 1, 1)),
        )

        originalShape = ThreshGradMatrix.shape
        ThreshGradMatrix = ThreshGradMatrix.flatten(order="F")

        ThreshGradMatrix[(num_labels)::num_labels] = ThreshGradMatrix.flatten(
            order="F"
        )[(num_labels)::num_labels] + np.multiply(
            errorDer[:, 1 : (num_labels - 1)], fGradients[:, 0 : (num_labels - 2)]
        ).sum(
            axis=0
        )

        ThreshGradMatrix = np.reshape(
            ThreshGradMatrix[:, np.newaxis], originalShape, order="F"
        )
