...
class NNOP(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        initial_Theta2 = self.__randInitializeWeights(
            self.getHiddenN() + 1, num_labels - 1
        )
        ...
        Theta1, Theta2 = self.__unpackParameters(
    ...
    def predict(self, test):
        ...
        a3 = np.multiply(
            np.where(np.append(projected, np.ones((m, 1)), axis=1) > 0.5, 1, 0),
    ...
    def __unpackParameters(
        ...
        Theta2 = np.reshape(
            nn_params[nTheta1:], (num_labels - 1, hidden_layer_size + 1), order="F"
        )
    ...
    def __nnOPCostFunction(
        ...
        p2 = (lambdaValue / m) * np.concatenate(
