...
class NNOP(BaseEstimator, ClassifierMixin):
	...
	def fit(self,X,y):
		...
		results_optimization = scipy.optimize.fmin_l_bfgs_b(func=self._nnop_cost_function, x0=initial_nn_params.ravel(),args=(n_features, self.n_hidden,
		...
		theta1, theta2 = self._unpack_parameters(self.nn_params, n_features, self.n_hidden, n_classes)
		self.theta1_ = theta1
		self.theta2_ = theta2
	...
	def predict (self, X):
		...
		a3 = np.multiply(np.where(np.append(projected, np.ones((n_samples,1)), axis=1)>0.5, 1, 0),
	...
	def _unpack_parameters(self, nn_params, n_features, n_hidden, n_classes):
		...
		theta2 = np.reshape(nn_params[n_theta1:], (n_classes-1,
		 n_hidden+1),order='F')
	...
	def _nnop_cost_function(self, nn_params, n_features, n_hidden,
	n_classes, X, Y, lambda_value):
		...
		theta1,theta2 = self._unpack_parameters(nn_params,n_features, n_hidden, n_classes)
		...
		# MSE
		J = np.sum((out-Y)**2).sum()/(2*n_samples) + lambda_value*p/(2*n_samples)
		...
		p2 = (lambda_value/n_samples) * np.concatenate((np.zeros((np.size(theta2, axis=0), 1)), theta2[:,1:]), axis=1)
